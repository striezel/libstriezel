...
int main(int argc, char** argv)
{
  ...
  {
    //construct file name
    const std::string fileName = tarDirectory + libthoro::filesystem::pathDelimiter + item.first;
    //existence check
    ...
    const bool isTar = libthoro::tar::archive::isTar(fileName);
  ...
  //All OK.
  std::cout << "Tests for libthoro::tar::archive::isTar() were successful." << std::endl;
  return 0;
}

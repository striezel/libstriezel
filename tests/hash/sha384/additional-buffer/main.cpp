...
int main(int argc, char** argv)
{
  ...
  for (auto && item : testData)
  {
    if (hashLongData || (std::get<0>(item)<= cShortMaximum))
    {
      ...
      {
        std::cout << "Error: Unable to allocate buffer of " << std::get<0>(item)
                  << " bytes!" << std::endl;
        return 1;
      }

      std::memset(buffer, std::get<1>(item), std::get<0>(item));
      //compute hash
      const SHA384::MessageDigest md_sha384 = SHA384::computeFromBuffer(buffer, std::get<0>(item)*8);
      delete[] buffer;
      buffer = nullptr;

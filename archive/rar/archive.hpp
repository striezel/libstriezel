...
{
  public:
     /** \brief constructor - opens a rar archive in read-only mode
      *
      * \param fileName  -  file name of the rar archive
    ...
    /** \brief checks whether a file may be a rar archive
     *
     * \param fileName  file name of the potential rar archive
     * \return Returns true, if the file seems to be a rar archive.
     *         Returns false, if not.
     */
    static bool isRar(const std::string& fileName);
  private:
    /** \brief apply format support for rar archives
     */
    void applyFormats();
}; //class
